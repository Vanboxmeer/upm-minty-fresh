...
const TankChatWidget = () => {
  ...
  return (
    <div className="fixed bottom-20 right-4 z-50 hidden md:block animate-[scale-in_0.3s_ease-out]">
      <div className="w-[380px] h-[520px] bg-card/95 backdrop-blur-lg border-2 border-primary/30 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        {/* Header */}
              ...
              <div className="space-y-2 w-full">
                {quickQuestions.map((q, i) => (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="w-full text-xs justify-start h-auto py-2 hover:border-primary hover:bg-primary/10"
                ...
                >
                  <div
                    className={`max-w-[85%] rounded-xl px-3 py-2 text-sm ${
                      message.role === "user"
                        ? "bg-gradient-to-br from-primary to-primary-electric text-white"
                      ...
                      <div className="prose prose-sm dark:prose-invert max-w-none">
                        <ReactMarkdown
                          components={{
                            ...
                            ul: ({ node, ...props }) => (
                              <ul className="mb-1 ml-3 list-disc text-sm" {...props} />
                            ),
                            strong: ({ node, ...props }) => (
        ...
        <div className="border-t border-primary/20 p-3 bg-card/50">
          <div className="flex gap-2">
            <Textarea
              ...
              className="min-h-[40px] max-h-[80px] resize-none text-sm border-primary/20"
              disabled={isLoading}
              maxLength={1000}
            />
            <Button
              ...
              size="icon"
              className="h-10 w-10 bg-primary hover:bg-primary-electric shrink-0"
            >
              {isLoading ? (
